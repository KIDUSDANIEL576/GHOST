...
export function QuickActions({ onDone }: { onDone?: () => void }) {
  ...
  return (
    <div className="flex items-center gap-1">
      {[
        { key: 'undo', icon: Undo2, label: 'Undo', color: 'hover:text-orange-300',
          fn: async () => {
        ...
        { key: 'restore', icon: RotateCcw, label: 'Last Working', color: 'hover:text-green-300',
          fn: async () => {
            const r = await invoke<any>('restore_last_working');
            toast[r.success ? 'success' : 'error'](r.success ? '✅ Restored' : r.error || 'No snapshot found');
          }},
        { key: 'snap', icon: Camera, label: 'Quick Snap', color: 'hover:text-blue-300',
          fn: async () => {
        ...
        <button key={a.key} onClick={() => run(a.key, a.fn)} disabled={busy === a.key}
          className={`flex items-center gap-1.5 px-3 py-1 text-xs text-gray-400 rounded transition-colors disabled:opacity-40 ${a.color} cursor-pointer`}>
